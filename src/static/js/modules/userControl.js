...
export const userControl = {
	...
	cache() {
		...
		this.changeWeightForm = document.querySelector('#changeWeight');
	...
	addListeners() {
		...
		this.changeWeightForm.addEventListener('submit', (event) => {
			...
			this.changeWeight(selectedItem, formData.get('weight'));
			app.clearFields(event.target)
		});
	...
	render() {
		...
		app.db.users.orderBy('weight').each(i => {
			let negativeCredit = false;
			if (i.credit < 0) negativeCredit = true;
	...
	async changeWeight(entry, weight) {
		status.add('delete');
		console.log("weight: " + weight);
		const userData = await app.db.users.get(entry);
		app.db.users.update(entry, {weight: weight}).then(function (updated) {
			if (updated)
				status.log('succes');
			else
				status.log("Nothing was updated - there were no friend with primary key: 2");
		});
		$('#modalChangeWeight').modal('hide');
		this.render();
	}
};
